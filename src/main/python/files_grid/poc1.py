...
def get_site_map_dict(url_param, path):
    site_map_dict = {}
...
def get_pages_from_site_map(url_param):
    site_map_dict = get_site_map_dict(url_param, "sitemap.xml")

    if not bool(site_map_dict):
        site_map_dict = get_site_map_dict(url_param, "sitemap_index.xml")
        if not bool(site_map_dict):
            print("site map not available for:" + url_param)
            return
...
for url in range(len(urlList)):
    ...
    siteMapDict = get_pages_from_site_map(urlList[url][0])
    if bool(siteMapDict):
